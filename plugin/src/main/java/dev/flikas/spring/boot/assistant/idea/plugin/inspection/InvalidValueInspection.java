...
public class InvalidValueInspection extends YamlInspectionBase {
  ...
  protected void visitKeyValue(
    ...
    if (yamlValue instanceof YAMLScalar scalar) {// A YAML scalar can map to a scalar value or a list of one element.
      PsiType elementType;
    ...
    } else if (yamlValue instanceof YAMLSequence sequenceValue) {
      if (!PsiTypeUtils.isCollection(project, type)) {
        holder.registerProblem(
            yamlValue,
    ...
    } else if (yamlValue instanceof YAMLMapping mappingValue) {
      ...
      log.warn("Unsupported yaml node type: " + PsiUtilCore.getElementType(yamlValue));
    }
  }
  ...
  private void validateValue(
    ...
    try {
      ...
      if (springConversionService.canConvert(String.class, valueClass)) {
