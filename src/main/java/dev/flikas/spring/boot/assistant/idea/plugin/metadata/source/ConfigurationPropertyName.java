...
public class ConfigurationPropertyName implements Comparable<ConfigurationPropertyName> {
  ...
  ConfigurationPropertyName(Elements elements) {
   ...
   * @since 2.3.1
   */
  static ConfigurationPropertyName ofIfValid(CharSequence name) {
    return of(name, true);
  }
  ...
  static Elements probablySingleElementOf(CharSequence name) {
    return elementsOf(name, false, 1);
  }


  static Elements elementsOf(CharSequence name, boolean returnNullIfInvalid) {
    return elementsOf(name, returnNullIfInvalid, ElementsParser.DEFAULT_CAPACITY);
  }


  static Elements elementsOf(CharSequence name, boolean returnNullIfInvalid, int parserCapacity) {
    if (name == null) {
    ...
    if (name.charAt(0) == '.' || name.charAt(name.length() - 1) == '.') {
      ...
      throw new IllegalArgumentException(
          "Invalid property name \"" + name + "\", which should not begin or end with dot(.)");
    }
    ...
    for (int i = 0; i < elements.getSize(); i++) {
      if (elements.getType(i) == ElementType.NON_UNIFORM) {
        ...
        throw new IllegalArgumentException("Invalid property name \"" + name + "\", which has invalid chars:"
  ...
  protected int compare(ConfigurationPropertyName n1, ConfigurationPropertyName n2) {
  ...
  static class ElementsParser {
    ...
    private void add(int start, int end, ElementType type, Function<CharSequence, CharSequence> valueProcessor) {
      ...
      if (valueProcessor != null) {
        ...
        if (resolvedElements.getSize() != 1) {
          throw new IllegalStateException("Resolved element must not contain multiple elements");
        }
